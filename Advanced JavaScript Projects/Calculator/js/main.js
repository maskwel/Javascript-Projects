//creares an object to keep track of values
const Calculator = {
...
function Input_Digit(digit) {
    ...
    if (Wait_Second_Operand === true) {
        ...
        //this overwrites Display_Value if the current value is 0 othervise it adds onto it
...
function Input_Decimal(dot) {
    //this ensures that accidental clicking of the decimal point doesn't cause bugh in the operation
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot) ) {
        //we are saying that if the Display_Value does contain a decimal point we want to add a decima point
...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed we convert the current number diplayed on the screento a number and
    //then store the result in Calculator.First_Operand if it doesn't already exist
    const Value_Of_Input = parseFloat(Display_Value);
    //checks if an operator already exists and if Wait_Second_Operand is true, then updates the operator
    //an exists from the function
    ...
    if (First_Operand == null) {
        ...
        //this will remove any trilyng 0's
...
keys.addEventListener('click', (event) => {
    ...
    //esures that AC clears all inputs from the Calculator screen
